...
`_harness/bin/harness-cli query tools` and documented in `_harness/docs/TOOL_REGISTRY.md`.
...
The advisory result from `_harness/bin/harness-cli score-context <trace-id>`. It
compares a trace's recorded `files_read` against compiled context rules and
retrieval triggers.

## Entropy Score

The drift score printed by `_harness/bin/harness-cli audit`. Lower is better. It
...
A structured recommendation generated by `_harness/bin/harness-cli propose` from
...
The SQLite database and CLI (`_harness/bin/harness-cli`) that stores operational
