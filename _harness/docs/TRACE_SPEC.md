...
The current schema lives in `_harness/schema/001-init.sql` under the `trace`
table, extended by `_harness/schema/006-next-action.sql` (the `next_action`
resume hint).
...
| `files_read`       | TEXT    | Standard+                                       | JSON array text of paths or command names. With the current CLI, pass a comma-separated list.                                                                 | `["PHASE2.md","_harness/docs/HARNESS.md","_harness/bin/harness-cli query matrix"]`               |
| `files_changed`    | TEXT    | Standard+                                       | JSON array text of changed file paths. With the current CLI, pass a comma-separated list; omit only when no files changed.                                    | `["_harness/docs/TRACE_SPEC.md","_harness/docs/HARNESS.md"]`                                             |
...
_harness/bin/harness-cli trace \
  ...
  --read "PHASE2.md,_harness/docs/HARNESS.md,_harness/schema/001-init.sql" \
  --changed "_harness/docs/TRACE_SPEC.md,_harness/docs/HARNESS.md" \
  --friction "none"
```
...
- Review the score printed automatically by `_harness/bin/harness-cli trace`. Use
  `_harness/bin/harness-cli score-trace --id N` when re-checking a specific
  historical trace.
